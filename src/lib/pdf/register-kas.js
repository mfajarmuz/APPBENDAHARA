...
export function exportRegisterKasPdf2(data, cashUnits, monthIndex, year, customDate = null) {
  const doc = new jsPDF({ orientation: 'p', unit: 'mm', format: 'a4' })
  const settings = useStore.getState().settings
  const monthName = getMonthName(monthIndex)
  const lastDay = customDate ? new Date(customDate).getDate() : new Date(year, monthIndex + 1, 0).getDate()
  ...
  doc.text('3', col1, curY)
  const wrapped3 = doc.splitTextToSize('Kertas Berharga dan Hak yang diizinkan Ordonansi / SP2D, Wesel, Cek, Saldo Bank, Materai, dan sebagainya ............................................................', 128)
  doc.text(wrapped3, col1 + 4, curY)
  const item3Value = data.saldo - sumCash
  doc.text('Rp.', col3, curY + ((wrapped3.length - 1) * 4))
  doc.text(formatRupiah(item3Value).replace('Rp', '').trim(), 190, curY + ((wrapped3.length - 1) * 4), { align: 'right' })
  ...
  if (curY > 240) { doc.addPage(); curY = 20 }
  doc.text(`${settings.lokasi || ''}, ${fullDate}`, 145, curY); curY += 10
  doc.text('BENDAHARA PENGELUARAN PEMBANTU,', col1, curY + 4)
  doc.text('Pemeriksa,', 145, curY)
  doc.text(settings.kpa_jabatan || 'Kuasa Pengguna Anggaran,', 145, curY + 4); curY += 25
  
  doc.setFont('helvetica', 'bold')
  doc.text(settings.bpp_nama || '', col1, curY)
  doc.text(settings.kpa_nama || '', 145, curY)
  curY += 4
  
  doc.setFont('helvetica', 'normal')
  doc.text(`NIP. ${settings.bpp_nip || ''}`, col1, curY)
  doc.text(`NIP. ${settings.kpa_nip || ''}`, 145, curY)

  doc.save(`Register_Penutupan_Kas_Bulanan_2_${monthName}_${year}.pdf`)
}
