...
export function exportRPPUAPdf2(data, filterBulan, year = new Date().getFullYear(), customDate = null) {
  const doc = new jsPDF({ orientation: 'p', unit: 'mm', format: 'a4' })
  const settings = useStore.getState().settings
  const displayDate = formatTanggal(customDate || new Date(year, filterBulan + 1, 0))
  ...
  let signY = doc.lastAutoTable.finalY + 15; if (signY > 230) { doc.addPage(); signY = 20 }
  doc.setFontSize(9); doc.setFont('helvetica', 'normal'); doc.text(`${settings.lokasi || ''}, ${displayDate}`, 140, signY)
  doc.text(settings.bpp_jabatan || 'Bendahara Pengeluaran Pembantu,', 14, signY + 10)
  doc.text('Pemeriksa,', 140, signY + 5)
  doc.text(settings.kpa_jabatan || 'Kuasa Pengguna Anggaran,', 140, signY + 10)
  doc.setFont('helvetica', 'bold')
  doc.text(settings.bpp_nama || '', 14, signY + 35)
  doc.text(settings.kpa_nama || '', 140, signY + 35)
  doc.setFont('helvetica', 'normal')
  doc.text(`NIP. ${settings.bpp_nip || ''}`, 14, signY + 39)
  doc.text(`NIP. ${settings.kpa_nip || ''}`, 140, signY + 39)
  doc.save(`RPPUA_Bulanan_2_${year}.pdf`)
}
