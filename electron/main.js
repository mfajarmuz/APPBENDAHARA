...
// Inisialisasi Supabase (dijalankan di main process untuk keamanan)
const supabaseUrl = process.env.SUPABASE_URL;
const supabaseKey = process.env.SUPABASE_ANON_KEY; // Pastikan ini diatur di file .env
...
function createWindow() {
  ...
  // Load React app (dari Vite server jika dev, atau dari build dist jika production)
...
// ==========================================
// IPC HANDLERS - DATABASE (SUPABASE CRUD)
// ==========================================
...
ipcMain.handle('get-kode-rekening', async (event, subKegiatanId) => {
  ...
  // Filter berdasarkan sub_kegiatan_id jika diberikan parameternya
  if (subKegiatanId) {
    query = query.eq('sub_kegiatan_id', subKegiatanId);
  }
...
ipcMain.handle('get-pengeluaran', async () => {
  // Tarik data beserta relasinya dalam satu kali query
